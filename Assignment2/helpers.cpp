 ...
 * Date: 4/15/2018
 * Description: helpers.cpp provides several functions to read numbers from
 * 		a file, sort them using a specified sorting algorithm, save
 * 		the resulting array to file, print the array, and test the
...
void readInputFile(int sortType) {
	...
	while ( getline(inputFile, nextLine) ) {
		...
		//cout << "Character read: " << nextChar << endl;
		...
		// Stooge sort
		stoogeSort(myNumbers, 0, (numCount - 1));
	
		//cout << "Sorting Complete for array: ";
		//printArray(myNumbers, numCount);
		// Save the sorted array to the output file
		saveArray(sortType, myNumbers, numCount, true);
	    }
...
void saveArray(int sortId, int *nums, int size, bool newLine) {
	...
	outFile.open("stooge.out", ios::app);
    }
...
void runtimeTest(float *results, int testCount, int testId) {
	...
	for (int j = 0; j < 5; j++) {
		...
		// Use stooge sort to test the array
...
/*************************************************************************************************
 * Description: stoogeSort demonstrates STOOGESORT, a recursive sorting algorithm that sorts the 
 * 		first 2/3's of an array, followed by sorting the second 2/3's, and finally sorts
 * 		the first 2/3's again, resulting in a fully sorted array
 * Parameters: 'nums' is a pointer to an array of integers
 * 	       'firstIndex' is the first index to be sorted
 * 	       'lastIndex' is the last index to be sorted
 * Pre-conditions: 'nums' exists
 * Post-conditions: 'nums' is sorted in ascending order
 *************************************************************************************************/
void stoogeSort(int *nums, int firstIndex, int lastIndex){
    // Validate parameters
    assert(nums != NULL);
    assert(lastIndex >= firstIndex);    

    //cout << "Stooge sort started..." << endl;
    ...
    // Check if size = 2 & value of firstIndex > value of lastIndex
    if ( (size == 2) && (nums[firstIndex] > nums[lastIndex]) ){
        // If yes, swap value of first index and last index
    ...
    // Check if size > 2
        ...
        // Calculate 2/3 index
        int m = static_cast<int>( ceil(static_cast<float>(size) * 2.0 / 3.0) );
        // Recursively call stoogeSort from firstIndex to 2/3s index - 1
        //cout << "*** First recursive sort, from: " << firstIndex << ", to: " << (m - 1) << endl;
        stoogeSort(nums, firstIndex, firstIndex + m - 1);
        // Recursively call stoogeSort from (lastIndex - 2/3 index) to size - 1
        //cout << "*** Second recursive sort, from: " << (lastIndex - m + 1) << ", to: " << lastIndex << endl;
        stoogeSort(nums, lastIndex - m + 1, lastIndex);
        // Recursively call stoogeSort from firstIndex to 2/3s index - 1
        //cout << "*** Final recursive sort, from: " << firstIndex << ", to: " << (m - 1) << endl;
        stoogeSort(nums, firstIndex, firstIndex + m - 1);
    }
}
